...
function enumerateAllHands() {
  ...
  for (c0 = 1; c0 < 47; c0++) {
    u0 = evaluator.evaluateCard(53 + c0);
    for (c1 = c0 + 1; c1 < 48; c1++) {
      u1 = evaluator.evaluateCard(u0 + c1);
      for (c2 = c1 + 1; c2 < 49; c2++) {
        u2 = evaluator.evaluateCard(u1 + c2);
        for (c3 = c2 + 1; c3 < 50; c3++) {
          u3 = evaluator.evaluateCard(u2 + c3);
          for (c4 = c3 + 1; c4 < 51; c4++) {
            u4 = evaluator.evaluateCard(u3 + c4);
            for (c5 = c4 + 1; c5 < 52; c5++) {
              u5 = evaluator.evaluateCard(u4 + c5);
              for (c6 = c5 + 1; c6 < 53; c6++) {
                handTypeSum[evaluator.evaluateCard(u5 + c6) >> 12]++;
                count++;
              }
            }
          }
  ...
  var testCount = 0;
  for (var index = 0; index < 10; index++) testCount += handTypeSum[index];
